/* ============================================================
   PERFECT CLAUDE v22 — ADMIN OVERLAY
...
(function () {
  ...
  function buildExtra(e) {
    ...
    var title =
      lang === "en" ? e.title_en || e.title_ar : e.title_ar || e.title_en;
    var body = lang === "en" ? e.body_en || e.body_ar : e.body_ar || e.body_en;
    var ltxt =
      lang === "en"
        ? e.link_text_en || e.link_text_ar
        : e.link_text_ar || e.link_text_en;
    ...
    if (e.image_url) {
      imgHTML =
        '<div class="pc-img-wrap"><img class="pc-img" src="' +
        e.image_url +
        '" alt="" style="' +
        (SHAPE_css[e.image_shape] || SHAPE_css.rounded) +
        '"></div>';
    }
    ...
    var ctaHTML =
      e.link_url && ltxt
        ? '<a class="pc-cta" href="' + e.link_url + '">' + ltxt + "</a>"
        : "";
    ...
    if (e.layout === "split") {
      card.innerHTML =
        e.image_pos === "left" || e.image_pos === "top"
          ? imgHTML + textBlock
          : textBlock + imgHTML;
  ...
  function zoneAnchor(zone) {
    var hero = document.querySelector(
      '.hero, #hero, header.hero, [data-zone="hero"]'
    );
    var footer = document.querySelector(
      '.footer, footer, [data-zone="footer"]'
    );
    var main = document.querySelector("main") || document.body;
    if (zone === "top") return { parent: main, before: main.firstChild };
    if (zone === "after-hero" && hero && hero.parentNode)
      return { parent: hero.parentNode, before: hero.nextSibling };
    if (zone === "before-footer" && footer && footer.parentNode)
      return { parent: footer.parentNode, before: footer };
    return {
      parent: main,
      before: footer && footer.parentNode === main ? footer : null,
    };
  }
  ...
  function renderExtras(rows) {
    ...
    Object.keys(byZone).forEach(function (zone) {
      ...
      byZone[zone].forEach(function (e) {
        grid.appendChild(buildExtra(e));
      });
  ...
  function applyOverrides(rows) {
    ...
    rows.forEach(function (o) {
      var nodes;
      try {
        nodes = document.querySelectorAll(o.selector);
      } catch (e) {
        console.warn("[overrides] selector invalid:", o.selector);
        return;
      }
      ...
      nodes.forEach(function (n) {
        try {
          if (o.type === "hide") {
            ...
            if (n.tagName === "IMG")
              n.src = o.value || o.value_ar || o.value_en;
            else
              n.style.backgroundImage =
                'url("' + (o.value || o.value_ar || o.value_en) + '")';
          } else if (o.type === "attr") {
            if (o.attr_name) n.setAttribute(o.attr_name, o.value || "");
          } else if (o.type === "html") {
            var v =
              lang === "en"
                ? o.value_en || o.value_ar
                : o.value_ar || o.value_en;
            n.innerHTML = v || "";
          } else {
            /* text */
            var t =
  ...
  function load() {
    ...
    /* Extras */
    fetch("/api/page-extras/" + encodeURIComponent(page), {
    ...
    function fetchOverrides() {
      fetch("/api/content-overrides/" + encodeURIComponent(page), {
        credentials: "same-origin",
      })
        .then(function (r) {
          return r.ok ? r.json() : [];
  ...
  document.addEventListener("DOMContentLoaded", function () {
    if (
      !document.cookie.match(/pc_token=/) &&
    ...
    bar.style.cssText =
      "position:fixed;bottom:18px;inset-inline-start:18px;z-index:99999;padding:10px 18px;background:#00f0ff;color:#000;border-radius:30px;font-weight:700;text-decoration:none;box-shadow:0 6px 24px rgba(0,240,255,.5);font-family:Cairo,sans-serif;font-size:13px";
