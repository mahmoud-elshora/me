...
function signToken(user) {
  return jwt.sign({ u: user.username, r: "admin" }, JWT_SECRET, {
    expiresIn: "7d",
  });
}
...
function getVisitorsCached() {
  ...
  if (now - _visitorsCache.updatedAt < CACHE_TTL_MS) {
    return Promise.resolve({
      today: _visitorsCache.today,
      total: _visitorsCache.total,
    });
  }
  ...
  getDB()
    .then(function (db) {
      return db
        ...
        .find({ approved: true, hidden: { $ne: true } })
        .toArray();
    })
    .then(function (rows) {
      res.json(rows);
...
app.put("/api/testimonials/approve/:id", verifyAdmin, function (req, res) {
  getDB()
    .then(function (db) {
      return db
        .collection("testimonials")
        .updateOne({ id: req.params.id }, { $set: { approved: true } });
    })
    .then(function () {
      res.json({ ok: true });
    })
...
app.put("/api/testimonials/:id", verifyAdmin, function (req, res) {
  ...
  getDB()
    .then(function (db) {
      return db
        .collection("testimonials")
        .updateOne({ id: req.params.id }, { $set: $set });
    })
    .then(function (r) {
...
app.put("/api/community/approve/:id", verifyAdmin, function (req, res) {
  getDB()
    .then(function (db) {
      return db
        .collection("community")
        .updateOne({ id: req.params.id }, { $set: { published: true } });
    })
    .then(function () {
      res.json({ ok: true });
    })
...
app.get("/api/page-extras/:page", function (req, res) {
  ...
  getDB()
    .then(function (db) {
      return db
        .collection("page_extras")
        .find({ page: p, visible: true })
        .sort({ order: 1 })
        .toArray();
    })
    .then(function (rows) {
...
app.post("/api/admin/page-extras", verifyAdmin, function (req, res) {
  ...
  getDB()
    ...
    .then(function () {
      res.json({ ok: true, id: doc.id, doc: doc });
    })
    .catch(function (e) {
...
app.put("/api/admin/content-overrides/:id", verifyAdmin, function (req, res) {
  ...
  getDB()
    .then(function (db) {
      return db
        .collection("content_overrides")
        .updateOne({ id: req.params.id }, { $set: doc });
    })
    .then(function (r) {
      res.json({ ok: r.matchedCount > 0 });
    })
...
app.delete(
  "/api/admin/content-overrides/:id",
  verifyAdmin,
  function (req, res) {
    getDB()
      .then(function (db) {
...
if (require.main === module) {
  ...
  app.listen(PORT, function () {
    console.log("Perfect Claude v23 running on http://localhost:" + PORT);
  });
}
